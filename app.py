...
# ---------------------------------------------------------
# 1. Konfigurasi Halaman & Custom CSS (Aman & Stabil)
# ---------------------------------------------------------
st.set_page_config(
...
if btn_analyze:
    if not nama.strip() or not kelas.strip() or not asal_sekolah.strip() or not no_hp.strip():
        ...
        st.error("Konfigurasi Secret GROQ_API_KEY belum terpasang.")
    else:
        with st.spinner("SIKUNTUL sedang menyusun analisis..."):
            try:
                ...
                4. **Rincian Biaya**:
                   Tampilkan rincian biaya Pendidikan TA 2026/2027 Universitas Nasional Karangturi HANYA menggunakan tabel Markdown 3 kolom berikut secara persis tanpa menambahkan baris Total per Tahun:

                   | Komponen | Biaya (Rp) | Keterangan |
                   | :--- | :--- | :--- |
                   | SPI (1x bayar) | 4.500.000 | Dapat diangsur 3x sebelum PKKMB |
                   | Inisiasi (1x bayar) | 1.200.000 | Orientasi, kaos, jas almamater |
                   | Biaya per Semester | 5.500.000 | 20 SKS = 4.000.000 + Daftar Ulang = 1.500.000 |

                   *Catatan:*
                   - Biaya SKS praktikum: Rp 250.000 per SKS
                   - Biaya wisuda: Rp 1.750.000
                   - Diskon karyawan: 10% dari total biaya kuliah (tidak termasuk biaya wisuda).
                """
                ...
                # Kirim Payload ke Google Apps Script (jika ada)
                payload = {
                    "nama": nama,
                    "kelas": kelas,
